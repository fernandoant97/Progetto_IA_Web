...
def predict():
    ...
    predictions = []
    predictions.append(make_prediction(model1, image))
    predictions.append(make_prediction(model2, image))
    predictions.append(make_prediction(model3, image))
    predictions.append(make_prediction(model4, image))
...
def predict_webcam():
    ...
    image_url = f'/static/{image_name}'  # Aggiorna il percorso dell'immagine

    predictions = []
    predictions.append(make_prediction(model1, f'static/{image_name}'))
    predictions.append(make_prediction(model2, f'static/{image_name}'))
    predictions.append(make_prediction(model3, f'static/{image_name}'))
    predictions.append(make_prediction(model4, f'static/{image_name}'))

    #return render_template('index.html', prediction=predictions, image_url=image_url)
    #return jsonify({'prediction': predictions, 'image_url': image_url})
